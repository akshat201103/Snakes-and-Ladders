...
import platform
import collections
def find_shortest_path(board, boards, start, end):
        ...
        for roll in range(1, 7):
            ...
            if next_cell <= end and next_cell not in visited and next_cell != 0:
                ...
                if ",LT" in board[boards[next_cell][0]][boards[next_cell][1]]:
                    # Calculate the head of the ladder
                    ladder_tail_num = int(board[boards[next_cell][0]][boards[next_cell][1]].split(",LT")[1][0])
                    for key, value in boards.items():
                        if ",LH" + str(ladder_tail_num) in board[value[0]][value[1]]:
                            next_cell = key
                            break
                elif ",SH" in board[boards[next_cell][0]][boards[next_cell][1]]:
                    # Calculate the tail of the snake
                    snake_head_num = int(board[boards[next_cell][0]][boards[next_cell][1]].split(",SH")[1][0])
                    for key, value in boards.items():
                        if ",ST" + str(snake_head_num) in board[value[0]][value[1]]:
                            next_cell = key
                            break
...
def main():
    ...
    for player_num in range(1, num_players + 1):#creating list of players playing and bonus available to them
        ...
        players.append([0])
    basic_display(board, players_position,players,bonus_chance)n
    board_size=le(board)
...
def inside_board(player_num,players,players_position,board,boards,bonus_chance):
    ...
    if(bonus_chance[player_num]>3):
        print("!!!!    Oh No you have exceded the limit of receiving bonus chances so all bonus chances you earned are now cancelled    !!!!")
        bonus_chance[player_num]=0
    ...
    shortest_path = find_shortest_path(board, boards, start, end)
    if shortest_path:
